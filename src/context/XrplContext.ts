...
class XrplContext {
  ...
  public constructor(
    ...
    options: XrplContextOptions = {}
  ) {
    this.hpContext = hpContext;
    this.xrplApi = options.xrplApi || new evernode.XrplApi();
    this.voteContext =
      options.voteContext ||
      new VoteContext(this.hpContext, options.voteOptions);
    this.xrplAcc = new evernode.XrplAccount(address, secret, {
  ...
  public async getTransactionSubmissionInfo(
    timeout: number = TIMEOUT
  ): Promise<TransactionSubmissionInfo> {
    // Decide a sequence number and max ledger sequence to send the same transaction from all the nodes.
    const infos: TransactionSubmissionInfo[] = (
      await this.voteContext.vote(
        `transactionInfo${this.voteContext.getUniqueNumber()}`,
        [
          <TransactionSubmissionInfo>{
            sequence: await this.getSequence(),
            maxLedgerSequence: this.getMaxLedgerSequence(),
          },
        ],
        new AllVoteElector(this.hpContext.unl.list().length, timeout)
      )
    ).map((ob) => ob.data);

    return <TransactionSubmissionInfo>{
      sequence: infos.map((i) => i.sequence).sort()[0],
      maxLedgerSequence: infos
        .map((i) => i.maxLedgerSequence)
        .sort((a, b) => b - a)[0],
    };
  }
   ...
   */
  public async multiSignAndSubmitTransaction(
    transaction: any,
    options: MultiSignOptions = {}
  ): Promise<any> {
    const txSubmitInfo = await this.getTransactionSubmissionInfo(
      options.voteTimeout
    );
    if (!txSubmitInfo) throw "Could not get transaction submission info";
    ...
    if (!signerListInfo || !signerCount) throw "Could not get signer list";
    ...
    const elector = new MultiSignedBlobElector(
      signerCount,
      signerListInfo,
      options.voteTimeout || TIMEOUT
    );
    ...
    if (this.isSigner()) {
      ...
      signatures = (
        await this.voteContext.vote(electionName, [signature], elector)
      ).map((ob) => ob.data);
    } else {
      signatures = (
    ...
    const res = await this.submitMultisignedTx(transaction).catch(
      console.error
    );
    if (res?.result?.engine_result === "tesSUCCESS")
      console.log("Transaction submitted successfully");
    else if (
      res?.result?.engine_result === "tefPAST_SEQ" ||
      res?.result?.engine_result === "tefALREADY"
    )
      console.log("Proceeding with pre-submitted transaction");
    else
      throw res?.result?.engine_result
        ? `Transaction failed with error ${res.result.engine_result}`
        : "Transaction failed";

    return res.result;
  }

  public async generateNewSignerList(
    options: MultiSignOptions = {}
  ): Promise<[SignerListInfo, SignerPrivate]> {
    ...
    if (!signerCount) throw "Signer count cannot be empty.";

    const elector = new AllVoteElector(
      signerCount,
      options.voteTimeout || TIMEOUT
    );
    ...
    if (this.isSigner()) {
      ...
      const weight =
        options.weight ||
        curSignerList?.signerList.find((s) => s.account === curSigner?.account)
          ?.weight;

      if (!weight || !quorum) throw "Weight or Signer Quorum cannot be empty.";
    ...
    const tx = {
      ...
      SignerEntries: [
        ...signerListInfo.signerList
          ...
          .sort((a, b) =>
            a.SignerEntry.Account < b.SignerEntry.Account ? -1 : 1
          ),
      ],
    };
  ...
  public async renewSignerList(options: MultiSignOptions = {}): Promise<void> {
    const [signerListInfo, newSigner] = await this.generateNewSignerList(
      options
    );
    await this.setSignerList(signerListInfo, options);

    // Set the signer if this is a signer node.
    if (newSigner) this.multiSigner.setSigner(newSigner);
  }
  ...
  async addXrplSigner(
    ...
    if (pubkey === this.hpContext.publicKey) {
      ...
      signer = (await this.voteContext.subscribe(electionName, elector)).map(
        (ob) => ob.data
      )[0];
    }
    ...
    if (options.quorum) signerList.signerQuorum = options.quorum;
    await this.setSignerList(signerList!, options);

    if (newSigner) this.multiSigner.setSigner(newSigner);
  }

  async removeXrplSigner(
    pubkey: string,
    options: MultiSignOptions = {}
  ): Promise<void> {
    const elector = new AllVoteElector(1, options.voteTimeout || TIMEOUT);
    ...
    if (signerList && signer) {
      signerList.signerList = signerList.signerList.filter(
        (s) => s.account != signer.account
      );
      if (options.quorum) signerList.signerQuorum = options.quorum;
      await this.setSignerList(signerList!, options);
    }

    if (curSigner) this.multiSigner.removeSigner();
  }
  ...
  public async getSignerList(): Promise<SignerListInfo | null> {
    const accountObjects = await this.xrplAcc.getAccountObjects({
      type: "signer_list",
    });
    if (accountObjects.length > 0) {
      const signerObject = accountObjects.filter(
        (ob: any) => ob.LedgerEntryType === "SignerList"
      )[0];
      const signerList: Signer[] = signerObject.SignerEntries.map(
        (signer: any) => ({
      ...
      const res = {
        signerQuorum: signerObject.SignerQuorum,
        signerList: signerList,
      };
      return res;
    } else return null;
  ...
  public makeAmountObject(amount: any, currency: any, issuer: any) {
    ...
    const amountObj =
      currency == evernode.XrplConstants.XRP
        ? amount
        : {
            currency: currency,
            issuer: issuer,
  ...
  public async buyURIToken(
    uriToken: URIToken,
    memos: Memo[] = [],
    hookParams: HookParameter[] = [],
    options: MultiSignOptions = {}
  ): Promise<void> {
    const tx = {
      Account: this.xrplAcc.address,
    ...
    if (memos) tx.Memos = evernode.TransactionHelper.formatMemos(memos);

    if (hookParams)
      tx.HookParameters =
        evernode.TransactionHelper.formatHookParams(hookParams);
